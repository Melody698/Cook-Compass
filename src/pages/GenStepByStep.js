...
const GenStepByStep = () => {
  ...
  useEffect(() => {
    const fetchGeneratedRecipes = async () => {
      try {
        ...
        const res = await fetch(`http://localhost:5000/api/recipes?userId=${userId}`);
        const data = await res.json();
        const match = data.data?.find(r => r.id === parseInt(id));
  ...
  const handleReturn = () => navigate(`/gen-instructions/${id}`);
